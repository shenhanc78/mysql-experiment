...
int main(const int argc, const char *argv[]) {
  ...
  if (argc < 3) {
   ...
   double sp = sqrt(
     ( (data_1.size() - 1) * p1.second * p1.second +
       (data_2.size() - 1) * p2.second * p2.second ) / (data_1.size() + data_2.size() - 2));
   // T value
   double t_value = p3.first / (sp * sqrt((double)1 / (double)data_1.size() + (double)1 / (double)data_2.size()));
   // fprintf(stderr, "T-Value: %.3f\n", t_value);
   students_t dist(data_1.size() + data_2.size() - 2);
   // P = 2 * q;
   double q = cdf(complement(dist, fabs(t_value)));
   ...
   if (q * 2 <= 0.01) {
     ...
     snprintf(p_value, sizeof(p_value), "%.2e", q * 2);
   } else {
     fprintf(stderr, "P value      = %.2f\n", q * 2);
     snprintf(p_value, sizeof(p_value), "%.2f", q * 2);
   }
   if (q * 2 > 0.05) {
   ...
   vector<double> percent_diff_set;
   for (auto i = diff_set.begin(), j = data_1.begin(), e = diff_set.end(); i != e; ++i, ++j)
     percent_diff_set.push_back(*i / *j);
   double wp = confidence_interval(percent_diff_set);
   auto [diff_mean, diff_standard_deviation] = get_mean_and_standard_deviation(percent_diff_set);
   
   fprintf(stderr, "Percent   (95%% CI) = %.2f%% (± %.2f%%) (P = %s)\n", diff_mean * 100, wp * 100, p_value);
